import { assert, expectNever } from "./errors.ts";
import { createResolution, provideInjectionContext, useInjectionContext } from "./injection-context.ts";
...
import { isBuilder, type Registration, type RegistrationOptions, Registry } from "./registry.ts";
import { Scope } from "./scope.ts";
import { type Constructor, isConstructor, type Token, type TokenList } from "./token.ts";
...
export function createContainer({
  ...
  function instantiateClass<T extends object>(Class: Constructor<T>): T {
    ...
    assert(options.scope != Scope.Singleton, `unregistered class ${Class.name} cannot be resolved in Singleton scope`);
  ...
  function resolveScopedInstance<T>(registration: Registration<T>, instantiate: () => T): T {
    ...
    const cleanups = [
      ...
      !isBuilder(provider) && resolution.stack.push(provider, { provider, scope: Scope.Singleton }),
    ];
    try {
